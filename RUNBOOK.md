...
| --- | --- | --- | --- |
| 3.4.1 | Business impact assessment | Assets > Containers: asset criticality score and dynamic tag on the cnapp-demo container (tag rule on image repo `nelssec/cnapp-demo` or namespace `cnapp-demo`); vulnerabilities re-ranked by TruRisk | `prioritize_findings` reads the asset TruRisk score (riskScore 244 for the image and the `cnapp-demo` code asset on CA1), max QDS and any customer-assigned criticality straight from the Container Security API, and `.qualys/criticality.yaml` supplies criticality and internet exposure when the platform has none; owners from CODEOWNERS |
...
| 3.4.5 | Vulnerability analysis (QDS) | Open one QID on the asset, show QDS vs CVSS and the TruRisk contribution | Every vulnerability entry in the QScanner report carries `qdsScore`, Qualys `risk` (10-50), CVSS and threat-intel flags; `prioritize_findings` scores from QDS and lists `qualys_risk` per finding (Log4Shell QIDs 984157/986972: QDS 100, risk 50, P1 score 305) |
...
| 3.4.7 | Prioritization methodology | Same QDS/TruRisk view: severity, exploitability, asset criticality, exposure combined | `prioritize_findings` prints its methodology paragraph and per-finding reasons (QDS base, threat-intel boosts, privileged/exposure/criticality multipliers, P1-P4 tiers) plus a `trurisk` block (score, max QDS, criticality, source `qualys`, `override` via `trurisk_score`, or `unavailable` with the reason) so the ranking is anchored to the same TruRisk number TotalCloud shows |
...
Demo path for 3.4: open the container in TotalCloud, show criticality tag, TruRisk 244 and QDS ranking, filter privileged + internet-exposed + KEV, then switch to Devin/terminal and run `prioritize_findings` on the same image report: the `trurisk` block returns the same 244 from the Container Security API, and the top of the queue is the two Log4Shell QIDs (QDS 100, Qualys risk 50, P1) with reasons, before the image was ever deployed. Without a token the tool still ranks on QDS and threat intel and reports `trurisk.source=unavailable`.
